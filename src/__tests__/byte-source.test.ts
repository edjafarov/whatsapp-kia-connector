...
describe('makeWhatsappByteSource', () => {
  // base is the host's shared data root; the source reads the content-addressed
  // cache under <base>/whatsapp/media (mediaDir(base)).
  const base = fs.mkdtempSync(path.join(os.tmpdir(), 'wa-bs-'));
  const mediaCacheDir = path.join(base, 'whatsapp', 'media');
  fs.mkdirSync(mediaCacheDir, { recursive: true });
  const src = makeWhatsappByteSource(base);

  it('reads bytes from the cache by content_hash', async () => {
    fs.writeFileSync(path.join(mediaCacheDir, 'deadbeef'), 'PNGDATA');
    const r = await src.fetch({} as any, { content_hash: 'deadbeef' } as any);
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.bytes.toString()).toBe('PNGDATA');
  });

  it('returns gone when the file was cleaned up', async () => {
    const r = await src.fetch({} as any, { content_hash: 'missing' } as any);
    expect(r).toMatchObject({ ok: false, kind: 'gone' });
  });

  it('returns unavailable (retryable) on a transient FS error', async () => {
    // A non-ENOENT read failure must be retryable, not terminal. Point the
    // source at a directory named like the content_hash so fsp.readFile throws
    // EISDIR (a transient code) rather than ENOENT.
    fs.mkdirSync(path.join(mediaCacheDir, 'adir'));
    const r = await src.fetch({} as any, { content_hash: 'adir' } as any);
    expect(r).toMatchObject({ ok: false, kind: 'unavailable' });
  });

  it('falls back to the documents.content_hash column by id', async () => {
    // The real drain candidate (scheduler.ts) does NOT carry content_hash, so
    // the source must resolve it from the documents column via the doc id.
    const db = openTestDb();
    try {
      ...
      await db.run(
        `INSERT INTO documents (source,source_id,type,title,markdown,metadata,source_url,content_hash,created_at,ingested_at,updated_at)
         VALUES ('whatsapp','c:9','file','pic',NULL,'{}','','cafef00d','t','t','t')`,
      );
      const id = (
        await db.all(`SELECT id FROM documents WHERE content_hash='cafef00d'`)
      )[0].id as bigint;
      const r = await src.fetch(db, { documentId: id } as any);
      expect(r.ok).toBe(true);
